...
def get_ucf101():
    ...
    for label in sorted(os.listdir(str(folder))):
        files = os.listdir(os.path.join(str(folder), label))
        num = int(len(files)*0.85)
...
def get_ucf101_val():
    ...
    for label in sorted(os.listdir(str(folder))):
        ...
        num2 = int(len(files)*0.85)
        for fname in files[num:num2]:
            fnames.append(os.path.join(str(folder), label, fname))
            labels.append(label)
...
def get_ucf101_train(opt, dtype):
    ...
    return train_fnames, train_labels, all_class


def get_hmdb_train(opt, dtype):
...
def get_kinetics(dataset=''):
    sourcepath = '/root/data1/sty/SG/workplace/kinetics/'
